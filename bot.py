...
trusiki_list = ['трусы-шорты', 'плавки', 'панталоны', 'слип', 'кюлот', 'бикини', 'чикибоксеры', 'стринги', 'семейники',
	'боксеры', 'хипсы', 'джокстрэпы', 'брифы', 'трусы'
	] # Трусы

duel_list = ['кнб', 'жребий', 'борьба', 'наперегонки', 'члены', 'пистолеты', 'видеоигры', 'гачимучи'] # Виды поединков
...
duel_list3 = ['нанометров', 'милиметров', 'сантиметров', 'метров', 'километров', 'миль'] # Мера длины растояния наперегонки
duel_list5 = ['Glock 20','Glock 34','Bersa Thunder 380','FN Five-seveN','Melior','Пистолет Макарова','FEG PA-63','Mauser C96','Volkspistole',
	...
	] # Список пистолетов для дуэли
duel_list5b = ['плечо','колено','руку','живот','ногу'] # Список органов, в которые может попасть стрелок
duel_list6a = ['секунд','минут','часов','дней','недель','месяцев','лет','десятилетий','столетий','тысячелетий'] # Время игры в Майн
duel_list6b = ['строил дом','строил ферму','выращивал пшеницу','убивал свиней','убивал овец','убивал зомби','готовил еду',
	...
	] # Чем можно заняться в майнкрафте
duel_list6c = ['взрывом динамита','деревянным мечом','железным мечом','золотым мечом','стрелой','огненной стрелой','палкой',
	...
	] # Разного рода оружие из майнкрафта

news_list1 = ['Шок!', 'Внимание!']
news_list2 = ['Гитлер','Путин','Лунтик','Зеленский','Буратино','Трамп','Обама','Дед Мороз','Санта Клаус','Рикардо Милос',
	'Санитар','Каменщик-хлебоед','Анимешник','Священник','Лидер партии КПРФ','Лидер партии ЛДПР','Муссолини',
	'Школьник из Санкт-Петербурга','Медведев','Сталин','Ленин','Моцарт','Вокалист группы Rammstein','Грузчик',
	'Московский блогер','Репер из Воронежа','Омский музыкант','Дворник','Лукашенко','Порошенко','Нурминский','Миша Джинсы'
	] # Люди
news_list3 = ['кинул зигу','украл у твоего бати флешку, где есть видео, на котором ты вытираешь жопу, через час он выложит это в Интернет',
	'признан террористом','был нацистом','был фашистом','обоссал подъезд в Воронеже','приехал в Москву',
	'убил жену','съел деда','ел детей','устроил революцию в 1917 году','умер','записал новый альбом','вытер жопу',
	'прыгнул в окно','пропал без вести','стал отцом','рассказал стих','получил диплом. Порадуемся за него','окончил третий класс с двумя тройками',
	'упал (ему было больно)','рассказал анекдот','сел в тюрьму','вышел из тюрьмы','сделал двойное сальто',
	'стал членом Единой России','пукнул))0)','умер от передоза','не учился в школе','получил двойку по математике',
	'создал свой канал на YouTube','поймал покемона в храме','был барабанщиком группы Nirvana'
	] # Действие
...
def get_text_messages(message):
	...
	elif mes[0].lower() == '!новости':
		bot.send_message(message.chat.id, f'{ran(news_list1)} {ran(news_list2)} {ran(news_list3)}.')
